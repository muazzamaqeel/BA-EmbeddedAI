...
\begin{table}[H]
	\centering
	\caption{Performance comparison of evaluated GUI frameworks on the STM32N6570-DK platform}
	\label{tab:gui_performance_comparison}
	...
	\begin{tabularx}{0.9\textwidth}{
		...
		\toprule
		\textbf{Framework} & \textbf{Average Frame Rate (FPS)} & \textbf{Average CPU Load (\%)} \\
		\midrule
		\textbf{TouchGFX} &
		25–30 &
		65–75 \\
...
The performance values reported for TouchGFX in Table~\ref{tab:gui_performance_comparison} were derived from official STMicroelectronics documentation and related evaluations rather than direct measurement on the STM32N6570-DK as there were not solid projects having this justification. The reference platform for these results is the STM32F469-EVAL board, which features an Arm® Cortex®-M4 processor operating at 180~MHz and integrates the Chrom-ART (DMA2D) graphics accelerator. According to the TouchGFX performance documentation, the CPU load on this board decreases from approximately 82~\% to 4~\% when DMA2D acceleration is enabled, demonstrating the significant offloading potential of hardware-assisted rendering. While the STM32N6570-DK employs a far more advanced Cortex®-M55 processor running at up to 600~MHz and includes the next-generation NeoChrom GPU alongside DMA2D~v3, both platforms share similar graphical processing concepts. Consequently, extrapolated performance expectations indicate an average frame rate of 25–30~FPS with an estimated CPU utilization between 65~\% and 75~\% when running full-screen animations and layered widget updates at a 60~Hz refresh rate. These values align with observed characteristics of TouchGFX’s event-driven rendering model, where CPU load increases proportionally with screen invalidation complexity, animation frequency, and blending operations handled through the DMA2D pipeline.

\textbf{Source:} \url{https://support.touchgfx.com/docs/development/hardware-selection/hardware-components/hardware-selection-mcu}

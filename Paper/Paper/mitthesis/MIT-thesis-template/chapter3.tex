...
The performance values reported for TouchGFX in Table~\ref{tab:gui_performance_comparison} were derived from published documentation and independent evaluations rather than direct measurement on the STM32N6570-DK hardware. According to available research and technical analyses, TouchGFX achieves an average frame rate of around 25–30 FPS on mid- to high-end STM32 microcontrollers when full-screen rendering and multiple animated widgets are active. The relatively high CPU load, typically between 65 \% and 75 \%, results from the framework’s layered event-driven rendering model, which handles screen invalidation, widget drawing, and DMA2D scheduling within a single high-priority task. Although performance improves when only partial frame updates are used, the framework’s architecture inherently introduces additional overhead due to its abstraction layers and memory copying operations. These figures represent realistic performance expectations for TouchGFX-based applications using hardware acceleration and a 60 Hz display refresh on STM32N6-class devices.

\textbf{Source:} \url{https://support.touchgfx.com/docs/basic-concepts/performance}
